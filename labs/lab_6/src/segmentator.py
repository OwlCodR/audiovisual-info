...
class Segmentator():
    ...
    def segment(self, axis: str):
        images = ImagesProcessor.getInputPathsFromFolder(self.__inputFolderPath)
        for path in images:
            info = ImageInfo(path)
            img = info.image
            ...
            if axis == 'vertical':
                ...
                for i in range(0, len(segments), 2):
                    ...
                    cropped = img.crop((segments[i], 0, segments[i + 1], height))
                    cropped.save(
                        f"{self.__outputLettersFolderPath}/{axis}/{str(i // 2 + 1).zfill(2)}.{ext}"
                    )
            ...
            if axis == 'horizontal':
                ...
                for i in range(0, len(segments), 2):
                    name, ext = os.path.basename(path).split('.')
                    cropped = img.crop((0, segments[i], width, segments[i + 1]))
                    cropped.save(
                        f"{self.__outputLettersFolderPath}/{axis}/{os.path.basename(path)}"
                    )
